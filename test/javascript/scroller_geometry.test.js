...
// Ported from the source's geometry.test.ts. jsdom does not compute layout,
// so each element's rect, scroll position, and dimensions are stubbed
// directly. Rects are expressed in viewport (client) coordinates and shift
// with the owning viewport's scrollTop.
...
function createFixture(options) {
  ...
  const items = options.items.map((item) => {
    ...
    if (item.messageId) element.dataset.messageId = item.messageId

    element.dataset.scrollAnchor = item.scrollAnchor ? "true" : "false"
    // Item rects are in client coordinates: viewport-relative top shifted by
    // the viewport's own top so getElementTop adds back scrollTop correctly.
...
describe("getUnanchoredScrollAnchor", () => {
  ...
  it("returns null when every anchor has already been handled", () => {
    ...
    const handled = new WeakSet([items[0]])
